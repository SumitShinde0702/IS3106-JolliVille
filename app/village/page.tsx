...
const getTilePath = (tileNumber: number) => {
  ...
  return `/village-items/tiles/FieldsTile_${String(tileNumber).padStart(
    2,
    "0"
  )}.png`;
};
...
function VillagePage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    "houses" | "tents" | "decor" | "archer-towers"
  >("houses");
  ...
  const loadVillageLayout = async (userId: string) => {
    try {
      ...
      if (!layout) {
        console.log(
          "No existing layout found, creating new one for user:",
          userId
        );
      ...
      setTileNumbers(
        Array.from({ length: savedGridSize }, (_, row) =>
          Array.from({ length: savedGridSize }, (_, col) =>
            getTileNumber(row, col, savedGridSize)
          )
        ).flat()
      );
      ...
      const ownedItemsList = initialItems.filter(
        (item) => ownedItemIds.has(item.id) && !placedItemIds.has(item.id)
      );
  ...
  const handleExpandPlot = async () => {
    ...
    if (resources.plotSize >= MAX_PLOT_SIZE || resources.points < expansionCost)
      return;
    ...
    try {
      ...
      setTileNumbers(
        Array.from({ length: newGridSize }, (_, row) =>
          Array.from({ length: newGridSize }, (_, col) =>
            getTileNumber(row, col, newGridSize)
          )
        ).flat()
      );
      ...
      alert(
        `Plot expanded successfully! New size: ${newGridSize}x${newGridSize}`
      );
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 flex">
      ...
      >
        ...
        <div className="flex flex-wrap gap-1 mb-4">
          {(["houses", "tents", "decor", "archer-towers"] as const).map(
            (category) => (
              <button
                key={category}
          ...
          {categoryItems.map((item) => (
            ...
            >
              <div className="relative w-full h-24 flex items-center justify-center">
                <div
                  className={`relative w-full h-full ${getItemSizeClass(
                    item.id
                  )}`}
                >
                  <Image
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  disabled={
                    resources.plotSize >= MAX_PLOT_SIZE ||
                    resources.points <
                      calculateExpansionCost(resources.plotSize)
                  }
                ...
                >
                  Expand Plot ({calculateExpansionCost(resources.plotSize)}{" "}
                  points)
                  <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none bg-gray-800 text-white text-xs rounded px-2 py-1 -top-12 left-1/2 transform -translate-x-1/2 w-48 z-50 shadow-lg">
                    ...
                    Note: Expanding will increase your grid size, which may
                    slightly change the appearance of your layout.
                  </div>
                </button>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="relative w-[800px] mx-auto">
                    ...
                    >
                      ...
                      {/* Grid Items */}
                      {Array.from({
                        length: currentGridSize * currentGridSize,
                      }).map((_, index) => (
                        <div
                          key={index}
                        ...
                        >
                          ...
                          {gridItems[index] && (
                            <div
                              ...
                              draggable
                              onDragStart={(e) =>
                                handleDragStart(e, gridItems[index]!, index)
                              }
                            >
                              <div
                                className={`relative w-full h-full transform ${getItemSizeClass(
                                  gridItems[index]!.id
                                )}`}
                              >
                                <Image
      ...
      {instructionPopup.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md">
            ...
            <p className="mb-4">
              1. Drag items from the sidebar to place them in your village plot
              <br />
              2. To remove an item, drag it back to the sidebar
              <br />
      ...
      <style jsx global>{`
        ...
        .animated-element img {
          ...
          height: 100%;
          object-fit: cover;
          animation: spriteAnimation var(--sprite-duration, 1s)
            steps(var(--sprite-steps, 4)) infinite;
        }
...
export default withUserOnly(VillagePage);
